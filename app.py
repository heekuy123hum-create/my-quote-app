...
# --- CSS ตกแต่ง UI (เอามาจากเวอร์ชั่นดีไซน์สวยของคุณ) ---
st.markdown("""
<style>
...
def save_data(df, filename, key_col=None):
    ...
    if 'ลบ' in df_to_save.columns:
        ...
        # แล้วเอาคอลัมน์ 'ลบ' ออกก่อนบันทึกก็ได้ หรือจะเก็บไว้ก็ได้ (ในที่นี้เก็บไว้แต่รีเซ็ตเป็น False)
        df_to_save['ลบ'] = False
...
# ==========================================
# 3. PDF ENGINE (ดีไซน์ใหม่ + Logic 20 บรรทัด)
# ==========================================
def create_pdf(d, items_df, summary, sigs, remark_text, show_vat_line):
    ...
    # --- ตารางสินค้า (Logic สำคัญ: บังคับ 20 บรรทัด) ---
    pdf.set_y(85)
    ...
    # เตรียมข้อมูลที่คลีนแล้ว (เอาเฉพาะที่มีรายการ)
    ...
    def print_sum_row(label, value, bold=False, line=False):
        ...
        pdf.set_font(use_f, 'B' if bold else '', 12)
...
# ------------------------------------------------------------------
# TAB 1: Quotation (ดีไซน์สวย + ฟังก์ชันครบ)
# ------------------------------------------------------------------
with tab1:
    ...
    with b2:
        if st.button("🚀 บันทึกและพิมพ์ PDF", type="primary", use_container_width=True):
            ...
            # Check duplicate logic optional here
...
# ------------------------------------------------------------------
# TAB 2: ลูกค้า (ใช้โค้ดใหม่ของคุณ: ปุ่ม Save ปุ่มเดียว)
# ------------------------------------------------------------------
with tab2:
...
# ------------------------------------------------------------------
# TAB 3: สินค้า (ใช้โค้ดใหม่ของคุณ: ปุ่ม Save ปุ่มเดียว)
# ------------------------------------------------------------------
with tab3:
