...
def load_data():
    ...
    # [NEW] เพิ่มคอลัมน์ 'ลบ' สำหรับใช้ติ๊กเลือก (ถ้ายังไม่มี)
    if 'ลบ' not in st.session_state.db_customers.columns:
        st.session_state.db_customers.insert(0, 'ลบ', False)
    ...
    # [NEW] เพิ่มคอลัมน์ 'ลบ' สำหรับสินค้า
    if 'ลบ' not in st.session_state.db_products.columns:
        st.session_state.db_products.insert(0, 'ลบ', False)

# ฟังก์ชันบันทึกข้อมูลลง CSV (ปรับปรุงให้ตัดช่อง 'ลบ' ออกก่อนเซฟ ไฟล์จะได้ไม่เลอะ)
def save_data(df, filename):
    # สร้างตัวแปรชั่วคราวเพื่อเอาคอลัมน์ 'ลบ' ออกก่อนบันทึก
    df_to_save = df.copy()
...
# บันทึกรายการในตารางหน้าแรก (Session State สำหรับหน้า Quotation ไม่ต้องเซฟลงไฟล์)
...
# ==========================================
# 2. PDF ENGINE (ฉบับเต็ม ไม่ตัดทอน)
# ==========================================
def create_pdf(d, items_df, summary, sigs, remark_text, show_vat_line):
    ...
    # วนลูปสร้างแถว (คงไว้ 20 แถวตามเวอร์ชันเดิม)
    for i in range(20):
        if i < len(items_df):
            row = items_df.iloc[i]
...
with tab1:
    ...
    # 1.2 ส่วนดึงข้อมูลลูกค้าจาก Tab 2
    c_h1, c_h2 = st.columns([1, 1])
    with c_h1: st.subheader("👤 ข้อมูลลูกค้า")
    with c_h2: 
        # ดึงรายชื่อลูกค้าล่าสุดจาก Session State
    ...
    # 1.3 ส่วนตารางสินค้าและ Auto-fill จาก Tab 3
    st.subheader("📦 รายการสินค้า")
    
    # ป้องกัน Error JSON NAN โดยการกรองค่า None/NaN ออกจาก List
    raw_p_codes = st.session_state.db_products['รหัสสินค้า'].tolist()
    p_codes = [str(x) for x in raw_p_codes if pd.notna(x) and str(x).lower() != 'nan' and str(x).strip() != '']
    
    # ใช้ค่าจาก Session State เป็นตัวตั้งต้น
    current_df = st.session_state.grid_df

    # แทนที่ค่า NaN ทั้งหมดใน DataFrame ด้วย 0 หรือค่าว่างก่อนส่งเข้า data_editor
    ...
    # --- ส่วนสำคัญ: LOGIC ตรวจสอบและ Auto-fill อัตโนมัติ ---
    needs_rerun = False
    
    # วนลูปดูว่ามีแถวไหนเปลี่ยน "รหัสสินค้า" แต่ข้อมูลยังไม่มาหรือไม่
    for idx, row in edited_df.iterrows():
        code = row['รหัสสินค้า']
        # ถ้ามีรหัสสินค้า และรหัสอยู่ในฐานข้อมูล
        if code and code in p_codes:
            # ดึงข้อมูลสินค้าต้นฉบับ
            found_prod = st.session_state.db_products[st.session_state.db_products['รหัสสินค้า'].astype(str) == str(code)]
            
            if not found_prod.empty:
                p_info = found_prod.iloc[0]
                # เช็คว่าชื่อสินค้าในตาราง ไม่ตรงกับฐานข้อมูล (แสดงว่าเพิ่งเปลี่ยนรหัส หรือยังไม่ได้ดึงค่า)
                # เราจะอัปเดตทับเลย เพื่อให้ User เห็นข้อมูลทันที
                if row['รายการ'] != p_info['รายการ']:
                    ...
                    needs_rerun = True  # สั่งให้รีเฟรชหน้าจอ

    # ถ้ามีการแก้ไขข้อมูลอัตโนมัติ ให้บันทึกกลับ Session State แล้ว Rerun ทันที
    if needs_rerun:
        st.session_state.grid_df = edited_df
        st.rerun()
    else:
        # ถ้าไม่มีการแก้ Auto-fill ก็บันทึกตามปกติ (เช่น User แก้จำนวนเงินเอง)
        st.session_state.grid_df = edited_df

    # 1.4 การคำนวณเงิน (ใช้ข้อมูลล่าสุดจาก edited_df)
...
with tab2:
    ...
    with c_btn1:
        # ปุ่มลบสำหรับลูกค้า (แก้ไขแล้วให้เซฟลงไฟล์ทันที)
        if st.button("🗑️ ลบรายการที่เลือก (ลูกค้า)", type="secondary", use_container_width=True):
            # 1. กรองแถวที่ไม่เอาออก
            new_df = edited_customers[edited_customers['ลบ'] == False]
            # 2. บันทึกลงไฟล์ทันที (กันข้อมูลกลับมา)
            save_data(new_df, CUST_FILE)
            # 3. อัปเดตตัวแปรหลัก
            st.session_state.db_customers = new_df
            # 4. สั่งให้โปรแกรม "จบงาน" และแสดงผลใหม่ทีเดียว
            st.rerun()

    with c_btn2:
        # ปุ่ม Save แยกต่างหาก (แก้ปัญหารีเฟรชรัวๆ)
        if st.button("💾 บันทึกการเปลี่ยนแปลง (ลูกค้า)", type="primary", use_container_width=True):
            save_data(edited_customers, CUST_FILE)           # ยุบโค้ดเซฟไปไว้ในฟังก์ชัน (บรรทัดเดียวจบ)
            st.session_state.db_customers = edited_customers # สั่งให้ RAM จำค่าใหม่ทันที
            st.success("✅ บันทึกข้อมูลลูกค้าลงไฟล์เรียบร้อย!")
            st.rerun()                                       # บังคับหน้าจอรันใหม่เพื่อดึงค่าล่าสุดมาโชว์

# --- TAB 3: ฐานข้อมูลสินค้า (มีปุ่มลบสำหรับมือถือ) ---
with tab3:
    ...
    with p_btn1:
        # [NEW] ปุ่มลบสินค้า - กดทีเดียวหายถาวร
        if st.button("🗑️ ลบรายการที่เลือก (สินค้า)", type="secondary", use_container_width=True):
            # 1. กรองเอาเฉพาะแถวที่ไม่ได้ติ๊ก 'ลบ' (False)
            new_df_p = edited_products[edited_products['ลบ'] == False]
            
            # 2. บันทึกทับลงไฟล์สินค้าทันที (กันข้อมูลเก่าไหลกลับมา)
            save_data(new_df_p, PROD_FILE)
            
            # 3. อัปเดตข้อมูลใน RAM ของระบบ
            st.session_state.db_products = new_df_p
            
            # 4. สั่งรีเฟรชหน้าจอเพื่อแสดงข้อมูลล่าสุดที่ลบแล้ว
            st.rerun()

    with p_btn2:
        # ปุ่มบันทึกสำหรับหน้าสินค้า
        if st.button("💾 บันทึกการเปลี่ยนแปลง (สินค้า)", type="primary", use_container_width=True):
            # 1. บันทึกข้อมูลที่แก้ไขลงไฟล์ CSV ถาวร
            save_data(edited_products, PROD_FILE) 
            
            # 2. อัปเดตหน่วยความจำ (RAM) ให้จำค่าใหม่ล่าสุด
            st.session_state.db_products = edited_products
            
            # 3. โชว์ข้อความว่าสำเร็จ
            st.success("✅ บันทึกข้อมูลสินค้าลงไฟล์เรียบร้อย!")
            
            # 4. สั่งรีเฟรชหน้าจอทีเดียวเพื่อให้ทุกอย่างอัปเดต
            st.rerun()
