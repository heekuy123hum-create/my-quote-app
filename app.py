...
# --- 1. CONFIG ---
st.set_page_config(page_title="ระบบใบเสนอราคา SIWAKIT", layout="wide")
...
# --- 2. PDF ENGINE (จัดตามต้นฉบับ 595.pdf) ---
def create_pdf(d, items_df, summary, sigs):
    ...
    for ext in ['png', 'jpg', 'jpeg']:
        ...
        if os.path.exists(logo_file):
            pdf.image(logo_file, x=10, y=10, w=22)
            break
    
    # 2. ข้อมูลบริษัทเรา (อยู่ที่เดิม ต่อจากโลโก้)
    pdf.set_xy(35, 10)
    ...
    pdf.cell(55, 14, "", 1, 0) # วาดกรอบสี่เหลี่ยม
    pdf.set_xy(146, 11)
    pdf.multi_cell(53, 6, f"เลขที่: {d['doc_no']}\nวันที่: {d['doc_date']}", 0, 'L')

    # 4. ข้อมูลเงื่อนไขอื่นๆ (อยู่ที่เดิม นอกกรอบ)
    pdf.set_xy(145, 25)
    pdf.set_font(use_f, '', 12)
    pdf.multi_cell(55, 6, f"กำหนดส่ง: {d['due_date']}\nเครดิต: {d['credit']} วัน\nราคาเสนอถึง: {d['exp_date']}", 0, 'L')
    ...
    for i in range(20): # บังคับ 20 บรรทัดตามต้นฉบับ
        if i < len(items_df):
            row = items_df.iloc[i]
            ...
            if val[1] == "" and i >= len(items_df): val = [""]*7 # ถ้าแถวว่างจริงๆ
        else: val = [""]*7
    ...
    # 8. ลายเซ็น (แบบที่สวย: เรียงข้างกันแนวขวาง ชิดขอบล่างสุด)
    pdf.set_y(-35) 
...
# --- 3. UI (ฟิลด์ครบทุกอย่าง) ---
st.subheader("📝 ออกใบเสนอราคา (Full Version)")
c1, c2 = st.columns(2)
with c1:
    my_comp = st.text_input("ชื่อบริษัทเรา", "SIWAKIT")
    my_addr = st.text_input("ที่อยู่เรา")
    my_tel = st.text_input("โทรศัพท์เรา")
...
if st.button("🚀 สร้างใบเสนอราคา (หน้าเดียวจบ)", type="primary", use_container_width=True):
