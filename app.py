...
# ฟังก์ชันโหลดข้อมูล (ถ้ามีไฟล์ CSV ให้โหลด ถ้าไม่มีให้สร้างค่าเริ่มต้น)
def load_data():
    # --- โหลดข้อมูลลูกค้า ---
...
with tab1:
    ...
    # === [FIX: แก้ไข Error JSON NAN สำหรับตารางหลัก] ===
    ...
    for col in cols_to_clean:
        if col in current_df.columns:
            # บังคับแปลงเป็นตัวเลข และแทนที่ NaN ด้วย 0.0 อีกครั้งเพื่อความชัวร์
    ...
    for idx, row in edited_df.iterrows():
        ...
        if code and code in p_codes:
            ...
            # ต้องกรองหาจาก DataFrame หลัก ไม่ใช่จาก p_codes ที่เป็น list
...
with tab2:
    ...
    # === [FIX: แก้ไขค่าว่าง NaN ก่อนแสดงผลป้องกัน Error JSON] ===
    if not st.session_state.db_customers.empty:
        # แทนที่ค่าว่างทั้งหมดด้วย string ว่าง เพื่อให้ JSON compliant
        st.session_state.db_customers = st.session_state.db_customers.fillna("")
    
    # ใช้ตัวแปรรับค่าก่อน แล้วค่อยเช็คความเปลี่ยนแปลง
    ...
    # ตรวจสอบว่ามีการเปลี่ยนแปลงหรือไม่ ถ้ามีให้บันทึกแล้ว Rerun เพื่อเคลียร์สถานะการเด้ง
    if not edited_customers.equals(st.session_state.db_customers):
        save_data(edited_customers, CUST_FILE) # บันทึกลงไฟล์
        st.session_state.db_customers = edited_customers # อัปเดต State
        st.rerun() # รีเฟรช
...
with tab3:
    ...
    if not st.session_state.db_products.empty:
        # แทนที่ค่าว่างในคอลัมน์ราคาด้วย 0.0
    ...
    if not edited_products.equals(st.session_state.db_products):
        save_data(edited_products, PROD_FILE) # บันทึกลงไฟล์
        st.session_state.db_products = edited_products # อัปเดต State
        st.rerun() # รีเฟรช
