...
with tab2:
    ...
    with c_btn1:
        ...
        if st.button("🗑️ ลบรายการที่เลือก (ลูกค้า)", type="secondary", use_container_width=True):
            # 1. กรองเอาแถวที่ติ๊กถูกออก
            new_df = edited_customers[edited_customers['ลบ'] == False]
            # 2. บันทึกทับลงไฟล์ CSV ทันที (เพิ่มบรรทัดนี้)
            save_data(new_df, CUST_FILE)
            # 3. อัปเดตในหน่วยความจำและรีเฟรชหน้าจอ
    ...
    with c_btn2:
        ...
        if st.button("💾 บันทึกการเปลี่ยนแปลง (ลูกค้า)", type="primary", use_container_width=True):
            save_data(edited_customers, CUST_FILE)      # 1. Save ลงไฟล์ (CSV)
            st.session_state.db_customers = edited_customers # 2. Update Memory
...
with tab3:
    ...
    with p_btn1:
        # ปุ่มลบสำหรับสินค้า (แก้ไขแล้วให้เซฟลงไฟล์ทันที)
