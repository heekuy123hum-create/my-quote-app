...
# --- TAB 2: ฐานข้อมูลลูกค้า (บันทึก Manual) ---
with tab2:
    st.header("👥 จัดการฐานข้อมูลลูกค้า")
    st.info("💡 วิธีใช้งาน: แก้ไขข้อมูลในตารางให้เสร็จเรียบร้อยก่อน (เพิ่มแถว/ลบแถว/แก้ไข) แล้วค่อยกดปุ่ม 'บันทึก' ด้านล่างทีเดียว")
    st.write("🗑️ **วิธีลบ:** คลิกที่หน้าแถวให้เป็นสีเข้ม แล้วกดปุ่ม Delete บนคีย์บอร์ด หรือคลิก icon ถังขยะ")
    ...
    edited_customers = st.data_editor(
        ...
        num_rows="dynamic", 
        use_container_width=True,
        key="db_cust_editor_manual" # เปลี่ยน Key เพื่อความชัวร์
    )
    
    # ปุ่ม Save แยกต่างหาก (แก้ปัญหารีเฟรชรัวๆ)
    if st.button("💾 บันทึกการเปลี่ยนแปลง (ลูกค้า)", type="primary"):
        save_data(edited_customers, CUST_FILE)     # 1. Save ลงไฟล์
        st.session_state.db_customers = edited_customers # 2. Update Memory
        st.success("✅ บันทึกข้อมูลลูกค้าเรียบร้อยแล้ว!")
        st.rerun() # รีเฟรชหน้าเพื่อให้ Tab 1 เห็นข้อมูลใหม่ทันที

# --- TAB 3: ฐานข้อมูลสินค้า (บันทึก Manual) ---
with tab3:
    st.header("📦 จัดการฐานข้อมูลสินค้า")
    st.info("💡 วิธีใช้งาน: แก้ไขข้อมูลให้เสร็จทั้งหมด แล้วกดปุ่ม 'บันทึก' ทีเดียว")
    st.write("🗑️ **วิธีลบ:** คลิกที่หน้าแถวให้เป็นสีเข้ม แล้วกดปุ่ม Delete บนคีย์บอร์ด หรือคลิก icon ถังขยะ")
    ...
    edited_products = st.data_editor(
        ...
        num_rows="dynamic", 
        use_container_width=True,
        key="db_prod_editor_manual" # เปลี่ยน Key เพื่อความชัวร์
    )
    ...
    if st.button("💾 บันทึกการเปลี่ยนแปลง (สินค้า)", type="primary"):
        ...
        st.success("✅ บันทึกข้อมูลสินค้าเรียบร้อยแล้ว!")
        st.rerun()
