...
def clear_data_callback():
    """ฟังก์ชันสำหรับปุ่มเคลียร์ข้อมูล (Reset Tab 1)"""
    # รีเซ็ตตารางสินค้า
    st.session_state.grid_df = pd.DataFrame(
        [{"รหัสสินค้า": "", "รายการ": "", "จำนวน": 0, "หน่วย": "", "ราคา": 0.0, "ส่วนลด": 0.0}] * 20
    )
    
    # รีเซ็ตฟิลด์ข้อมูลลูกค้าและท้ายบิล
    fields_to_clear = [
    ...
    # รีเซ็ตเลขที่เอกสารใหม่ (Generate ใหม่)
    if "doc_no_in" in st.session_state:
        st.session_state.doc_no_in = f"QT-{datetime.now().strftime('%Y%m%d')}-001"
    
    # รีเซ็ตตัวเลือก Dropdown
    if "cust_selector_tab1" in st.session_state:
...
# ------------------------------------------------------------------
with tab1:
    # ปุ่ม Clear ข้อมูลอยู่ด้านบนขวา
    h_col1, h_col2 = st.columns([0.85, 0.15])
    with h_col1:
        st.subheader("🏢 ข้อมูลผู้เสนอราคา")
    with h_col2:
        if st.button("🧹 เคลียร์ข้อมูล", use_container_width=True, type="secondary"):
            clear_data_callback()
            st.rerun()
    ...
    # อัปเดต grid_df ทันทีเพื่อให้การคำนวณข้างล่างเป็น Real Time
    ...
    # โค้ดส่วนนี้จะทำงานทันทีเมื่อมีการแก้ไขตารางสินค้า เพราะ st.data_editor จะ trigger rerun
    ...
    with f_col2:
        ...
        # VAT Checkbox: เมื่อคลิก Streamlit จะ rerun และคำนวณใหม่ทันที
    ...
    if st.button("🚀 สร้าง PDF + บันทึกประวัติ", type="primary", use_container_width=True):
        # --- ตรวจสอบเลขที่ซ้ำ (Check Duplicate Doc No) ---
        is_duplicate = False
        ...
        if is_duplicate:
            st.error(f"⚠️ ไม่สามารถบันทึกได้: เลขที่เอกสาร '{doc_no}' มีอยู่ในระบบแล้ว! กรุณาเปลี่ยนเลขที่ใหม่")
        else:
