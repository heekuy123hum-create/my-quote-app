...
# --- ส่วนของฐานข้อมูล (Session State) ---
# บันทึกข้อมูลลูกค้า
...
# ==========================================
# 2. PDF ENGINE (โครงสร้างเดิมเป๊ะๆ)
# ==========================================
...
with tab1:
    ...
    # Logic การดึงข้อมูลสินค้าเมื่อเลือกรหัส
    for idx, row in edited_df.iterrows():
        code = row['รหัสสินค้า']
        if code in p_codes:
            p_info = st.session_state.db_products[st.session_state.db_products['รหัสสินค้า'] == code].iloc[0]
            # เติมข้อมูลถ้าช่องยังว่าง
            if not row['รายการ']: edited_df.at[idx, 'รายการ'] = p_info['รายการ']
            if not row['หน่วย']: edited_df.at[idx, 'หน่วย'] = p_info['หน่วย']
            if to_num(row['ราคา']) == 0: edited_df.at[idx, 'ราคา'] = p_info['ราคา']
...
# --- TAB 2: ฐานข้อมูลลูกค้า (แก้บั๊กพิมพ์แล้วหาย) ---
with tab2:
    st.header("👥 จัดการฐานข้อมูลลูกค้า")
    # แก้บั๊ก: ให้คืนค่ากลับเข้า session_state โดยตรงทันที
    st.session_state.db_customers = st.data_editor(
        st.session_state.db_customers, 
        num_rows="dynamic", 
        use_container_width=True,
        key="db_cust_editor" # ใส่ key ให้ชัดเจน
    )
    st.info("💡 ข้อมูลในตารางนี้จะบันทึกทันทีและไปปรากฏที่ Tab 1")

# --- TAB 3: ฐานข้อมูลสินค้า (แก้บั๊กพิมพ์แล้วหาย) ---
with tab3:
    st.header("📦 จัดการฐานข้อมูลสินค้า")
    # แก้บั๊ก: ให้คืนค่ากลับเข้า session_state โดยตรงทันที
    st.session_state.db_products = st.data_editor(
        st.session_state.db_products, 
        num_rows="dynamic", 
        use_container_width=True,
        key="db_prod_editor" # ใส่ key ให้ชัดเจน
    )
    st.info("💡 เพิ่มสินค้าที่นี่ แล้วไปเลือก 'รหัสสินค้า' ในหน้าแรก ข้อมูลจะเด้งขึ้นมาเอง")
